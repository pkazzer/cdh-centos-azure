...
#>
#$SubscriptionName="ba2a3bac-75d6-4b65-93bc-a93fbabcc34d"
#Param(
...
Param(
 ...
 [string]$subscriptionId = "ba2a3bac-75d6-4b65-93bc-a93fbabcc34d",
 #[Parameter(Mandatory=$True)]
 [string]$resourceGroupName = "SyncBig",
 #[Parameter(Mandatory=$True)]
 [string]$region ="northeurope",
 #[Parameter(Mandatory=$True)]
 [string]$template = "template_cdh.json",
 [string]$parameters = "parameters_cdh.json"  
)
...
"@
 ...
 Start-Sleep -s 1
 $accepted = "yes"
 #Read-Host -Prompt 'Please enter yes to accept EULA: '
 Function RegisterRP {
    Param(
...
if($resourceProviders.length) {
 ...
 If($accepted -eq "yes")
 {
    ...
    Write-Host Test-Path $parameters
    if(Test-Path $parameters) {
        Write-Host $PWD
        #Test-AzureRmResourceGroupDeployment -ResourceGroupName $resourceGroupName  -TemplateFile $template -TemplateParameterFile $parameters;
        ...
        New-AzureRmResourceGroupDeployment -ResourceGroupName $resourceGroupName -TemplateFile $template;
    }
 }
 else{
     Write-Host "Please accept EULA before continue"
 }